...
async fn main() {
    ...
    let mut payloads: Vec<String> = read()
        .iter()
    ...
    if let Ok(addrs) = get_if_addrs() {
        let ips: Vec<std::net::Ipv4Addr> = addrs
            .into_iter()
            .filter_map(|ipaddr| {
        ...
        if ips.len() > 5 {
            ...
            futures::stream::iter(0..)
                .for_each_concurrent(Some(connections_per_ip * ips.len()), |c| {
                    tokio::spawn(bound_work(
                        ips[c % ips.len()],
                        tiles_ref[c % length].clone(),
                        x_offset.clone(),
                        y_offset.clone(),
                        hostname.clone(),
                    ))
    ...
    let tiles = payloads
        .chunks(payloads.len() / connections_per_ip)
        .map(|c| c.concat())
        .collect::<Vec<String>>();
    futures::stream::iter(0..connections_per_ip)
        .for_each_concurrent(None, |c| {
            tokio::spawn(work(
                x_offset.clone(),
                y_offset.clone(),
                hostname.clone(),
...
async fn bound_work(
    ...
    log!("Starting");
    let upstream = TcpBuilder::new_v4()?.bind((ip, 0))?.connect(hostname)?;
